...
//
// The ordinary convention: negative when this sorts before the other, positive
// when after, zero when equal - the same as strcmp and memcmp.
//
// It used to be the exact opposite, consistently: every branch negated and the
// memcmp operands swapped. Nothing read it wrongly, because the one place that
// takes its sign - operator<(CVariant) - compensated with "> 0". But a function
// called Compare returning the reverse of what every other Compare in C++
// returns is a trap, and the compensation looked like the bug.
//
// Changed together with that operator; they only make sense as a pair.
//
int CBuffer::Compare(const CBuffer& Buffer) const
{
	if(m_uSize < Buffer.m_uSize)
		return -1;
	if(m_uSize > Buffer.m_uSize)
		return 1;
	// memcmp with a null pointer is undefined even for a length of zero
	return m_uSize ? memcmp(m_pBuffer, Buffer.m_pBuffer, m_uSize) : 0;
}
