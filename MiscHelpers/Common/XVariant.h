...
class SERIAL_EXPORT XVariant : public CVariant
{
public:
	...
	//
	// UTF-8, and not because it is smaller.
	//
	// VAR_TYPE_UNICODE writes len * sizeof(wchar_t) raw bytes, and wchar_t is
	// two bytes on Windows and four on Linux. A string written by a Linux
	// server and read by a Windows viewer therefore came out as every second
	// character with a NUL between - "T a s k S e r v e r" - which is what
	// finally showed up the first time the two halves of this project spoke to
	// each other.
	//
	// The width of wchar_t is a property of the compiler, not of the data, and
	// it has no business on a wire. Reading is unaffected: AsQStr and
	// ToQString accept either form, so this is only about what is produced.
	//
	// The flag stays, for the one caller that has a reason - see the note on
	// the QStringList constructor below.
	//
	XVariant(const QString& String, bool bUtf8 = true) : CVariant(String.toStdWString(), bUtf8) {}
	...
	XVariant(const QStringList& List) : CVariant() {
		BeginList();
		foreach(const QString& Item, List)
			Write(Item.toStdWString(), true);	// utf-8, for the reason above
		Finish();
	}
