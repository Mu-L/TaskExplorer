...
public:
	...
	//
	// **explicit**, and that matters more than it looks.
	//
	// Without it this is an implicit conversion, and an implicit conversion to a
	// type that has operator< makes `a < b` viable for *any* pair of containers
	// - by converting both sides. Qt's metatype machinery probes exactly that
	// when it registers a signal argument, so a moc'd header that had seen this
	// class would instantiate CVariant(QSet<QString>), which recurses through
	// Write(QString) into Write(QChar) and fails to compile. The constraint
	// above does not help: QSet and QString both have begin() and end(); it is
	// QChar, two levels down, that does not.
	//
	// Explicit keeps every direct construction working - CVariant(List),
	// XVariant(List), Write(Id, List) - and only stops the compiler reaching for
	// it behind your back.
	//
	template<typename T, typename = std::void_t<decltype(std::declval<const T&>().begin()),
	                                            decltype(std::declval<const T&>().end())>>
	explicit CVariant(const T& List) : CVariant()
	{
		BeginList();
...
// "> 0" until CBuffer::Compare was given the ordinary sign convention; see there.
