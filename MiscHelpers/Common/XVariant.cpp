...
bool XVariant::FromQVariant(const QVariant& qVariant, QFlags Flags)
{
	...
	switch(qVariant.type())
	{
		...
		{
			//
			// Still on the caller's flag, and deliberately: this path serves
			// the local pipe to TaskHelper, where both ends are the same build
			// on the same machine and wchar_t is therefore the same width.
			// Nothing that leaves the machine comes through here - see the
			// note on XVariant's QString constructor, which is what the wire
			// uses and which is unconditionally UTF-8.
			//
			if (Flags & eUtf8) {
